...
public enum BoardLayers {
    ...
    DOCUMENT("Document", 48, 7, 1, false),
    REFERENCE("Reference", 49, 7, 1, false),
    T_DOCU("tDocu", 51, 7, 1, true),
    B_DOCU("bDocu", 52, 7, 1, true),
    SIM_RESULTS("SimResults", 88, 9, 1, false),
    SIM_PROBES("SimProbes", 89, 9, 1, false),
    MODULES("Modules", 90, 5, 1, false),
    NETS("Nets", 91, 2, 1, false),
    BUSSES("Busses", 92, 1, 1, false),
    PINS("Pins", 93, 2, 1, false),
    SYMBOLS("Symbols", 94, 4, 1, false),
    NAMES("Names", 95, 7, 1, false),
    VALUES("Values", 96, 7, 1, false),
    INFO("Info", 97, 7, 1, false),
    GUIDE("Guide", 98, 6, 1, false);
    ...
    private BoardLayers(String code, int number, int color, int fill, boolean showDefault) {
        this.code = code;
        this.number = number;
        this.color = color;
        this.fill = fill;
    ...
    public static boolean contains(int layerNumber) {
        for (BoardLayers ll : BoardLayers.values()) {
    ...
    public boolean isActive(int layerNum) {
        // Schem layers (88 - 98) appear in Board files but are marked as [ active="no" ]
        return layerNum >= 88;
    }
